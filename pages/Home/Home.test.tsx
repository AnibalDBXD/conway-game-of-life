...
describe('<Home />', () => {
  ...
  test("Should revive the cells by clicking a tip", () => {
    const { getByTestId, getByText, getByAltText } = render(<Primary {...Primary.args} />);
  ...
  test("Change the Columns&Rows and apply changes", () => {
    const { getByDisplayValue, getByText, getByTestId } = render(<Primary {...Primary.args} />);
    const columnsAndRowsInput = getByDisplayValue(DEFAULT_VALUES.COLUMUNS_AND_ROWS);
    ...
    act(() => {
      ...
      const lastCell = `${DEFAULT_VALUES.COLUMUNS_AND_ROWS - 1}${DEFAULT_VALUES.COLUMUNS_AND_ROWS - 1}`;
